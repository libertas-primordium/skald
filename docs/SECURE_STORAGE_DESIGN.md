...
The encrypted vault storage readiness decision is documented in [`ENCRYPTED_LOCAL_VAULT_STORAGE_READINESS_DECISION.md`](ENCRYPTED_LOCAL_VAULT_STORAGE_READINESS_DECISION.md). It admits only a later separate storage design or implementation branch; secure secret storage remains disabled, no secure secret storage success path exists, no vault container parser/writer exists, and no production persistence, sync, provider selection, signing/broadcasting, UI, endpoint, or mainnet behavior is enabled.
