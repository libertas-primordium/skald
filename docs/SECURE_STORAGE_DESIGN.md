...
Future Android secure storage should use Android Keystore-backed key wrapping or encryption for secret payload keys.
...
- Whether the Tink plus Bouncy Castle split stack selected for the current packaging probe should become the implementation candidate after license review, KDF calibration, Tink keyset/storage handling review, split-provider boundary review, and provider-boundary review, as tracked in [`ENCRYPTED_LOCAL_VAULT_DEPENDENCY_SPIKE.md`](ENCRYPTED_LOCAL_VAULT_DEPENDENCY_SPIKE.md) and [`ENCRYPTED_LOCAL_VAULT_KAT_VALIDATION.md`](ENCRYPTED_LOCAL_VAULT_KAT_VALIDATION.md). Desktop and Android runtime public KATs now pass for the probe stack, including Pixel 10 Pro XL / Android 16 runtime execution; the earlier install conflict and stale mDNS targeting failures were environment blockers, not KAT failures. The libsodium comparison rejected Lazysodium Java/Android for this branch due Android duplicate-JNA packaging failure and deferred IonSpin KMP pending isolated package/KAT review.
...
2. Complete the Tink/Bouncy Castle dependency review, KDF calibration, Tink keyset/storage handling review, split-provider boundary review, provider-boundary design, and platform wrapping choices through explicit review, or run a narrower replacement-stack review for IonSpin KMP or a Lazysodium/JNA conflict strategy.
