...
Linux users may later configure a custom vault storage directory through Skald Settings. That future custom root must be validated before use and must pass containment, symlink, permission, and durability review before persistence. It must not contain secrets, wallet labels, account labels, note text, or unsafe user-controlled path content, and it must not weaken encryption, storage atomicity, or durability fail-closed policy. Settings UI, settings persistence, custom-root validation, and custom-root use remain unimplemented.
