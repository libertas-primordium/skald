...
This pass admits no current vector material. The eleventh slower test-only step adds one commonTest-only public non-secret text-only metadata vector row in [`ENCRYPTED_LOCAL_VAULT_TEST_ONLY_PROVIDER_IDENTITY_KAT_PUBLIC_VECTOR_FIXTURE.md`](ENCRYPTED_LOCAL_VAULT_TEST_ONLY_PROVIDER_IDENTITY_KAT_PUBLIC_VECTOR_FIXTURE.md), the twelfth slower test-only step validates it in [`ENCRYPTED_LOCAL_VAULT_TEST_ONLY_PROVIDER_IDENTITY_KAT_PUBLIC_VECTOR_VALIDATION.md`](ENCRYPTED_LOCAL_VAULT_TEST_ONLY_PROVIDER_IDENTITY_KAT_PUBLIC_VECTOR_VALIDATION.md), and the thirteenth slower test-only step binds it to one non-executable KAT case in [`ENCRYPTED_LOCAL_VAULT_TEST_ONLY_PROVIDER_IDENTITY_KAT_CASE_BINDING.md`](ENCRYPTED_LOCAL_VAULT_TEST_ONLY_PROVIDER_IDENTITY_KAT_CASE_BINDING.md).
