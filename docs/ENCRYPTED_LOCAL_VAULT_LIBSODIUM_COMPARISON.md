...
| --- | --- | --- | --- |
| Tink plus Bouncy Castle split stack | `com.google.crypto.tink:tink-android:1.21.0`, `com.google.crypto.tink:tink:1.21.0`, `org.bouncycastle:bcprov-jdk18on:1.84` | Remains desktop KAT-validated candidate only | Android and Linux packaging already pass in this project, desktop JVM public KATs pass, and no native library artifacts were introduced. Android runtime KAT validation and provider-boundary review remain outstanding. |
...
- Misuse risk: split provider stack, Tink keyset semantics, Bouncy Castle lower-level KDF API, and Tink internal explicit-nonce API require a narrow future Skald-owned provider boundary before any vault implementation.
...
- KAT coverage: desktop JVM public vectors pass for RFC 9106 Argon2id and XChaCha draft AEAD_XCHACHA20_POLY1305. Android runtime KAT coverage remains outstanding.
...
Rejected for the current vault branch. It remains a possible future investigation target only if a narrower branch can resolve the JNA variant conflict without weakening the build, hiding duplicate classes, or broadening production crypto usage.
...
Deferred after comparison. A future branch may evaluate it only if the scope is explicitly narrowed to IonSpin packaging, KMP metadata compatibility, native library inventory, and public-vector mapping.

## Readiness Model Alignment

`VaultCryptoDependencyProbeCatalog` now records:

- `TinkBouncyCastleSplit`: `DesktopKatValidatedCandidate`.
- `LazysodiumJavaAndroid`: `RejectedForCurrentVault`.
- `IonSpinKmpLibsodium`: `DeferredAfterComparison`.

No candidate is production-approved. All candidates report `implementationEnabled = false`, `storageEnabled = false`, `productionPersistenceEnabled = false`, and `mainnetEnabled = false`.
...
4. Design a narrow Skald-owned disabled `VaultCryptoProvider` boundary.
5. Keep explicit nonce handling and key hierarchy inside that reviewed boundary only.
6. Calibrate Argon2id or the selected memory-hard KDF per platform/device class.
7. Define redaction, error, logging, and test-vector policy.
8. Verify lock/session lifecycle and memory-clearing limits.
9. Verify corruption and migration behavior.
10. Keep secure secret storage and secure metadata persistence disabled until all storage acceptance gates pass.
...
Keep the Tink plus Bouncy Castle split stack as the only currently packaged, desktop KAT-validated candidate, but do not promote it to production implementation. Its remaining blockers are Android runtime KAT validation, final dependency/license review, KDF calibration, and a narrow disabled provider-boundary design.

The next focused branch should either:

- add Android runtime KAT validation for the current Tink/Bouncy candidate,
