...
- Provider factory isolation evidence cannot add a non-disabled provider factory or constructor, expose provider handles or crypto objects, accept byte material, instantiate provider runtime, execute operations, run KATs, use randomness, run KDF/HKDF/HMAC/AEAD, change provider selection, or set `productionProviderSelectable=true`.
- Provider operation dispatch isolation evidence cannot add a provider operation dispatcher, dispatch to a non-disabled provider, invoke skeleton/candidate/factory/registry paths, expose provider handles or crypto objects, accept byte material, execute operations, run KATs, use randomness, run KDF/HKDF/HMAC/AEAD, compute header commitments, encrypt/decrypt records, wrap/unwrap keys, change provider selection, or set `productionProviderSelectable=true`.
- Provider KAT execution isolation evidence cannot add a provider KAT executor, invoke non-disabled provider/factory/registry/dispatcher/skeleton/candidate paths, expose provider handles or crypto objects, accept byte material, execute provider operations, run KATs, use randomness, run KDF/HKDF/HMAC/AEAD, compute header commitments, encrypt/decrypt records, wrap/unwrap keys, authorize provider selection, authorize `productionProviderSelectable=true`, convert public vector docs into readiness, or approve mainnet.
...
The later human-approved [`v1 canonical architecture decision`](ENCRYPTED_LOCAL_VAULT_V1_CANONICAL_ARCHITECTURE_DECISION.md) is authoritative where this historical crypto record describes alternatives or unresolved hierarchy details. Canonical v1 now requires Argon2id to derive a 32-byte KEK, XChaCha20-Poly1305 to unwrap a random 32-byte vault root, HKDF-SHA-256 separation into manifest/metadata-record/secret-record roots, distinct per-record keys, fresh 24-byte record nonces, and an independent backup/export hierarchy. Wrapped-root AEAD authenticates canonical public-header context; no separate persisted header HMAC is selected.

All canonical production crypto must use the selected Skald-owned `VaultCryptoProvider`. Existing direct prototype crypto remains noncanonical and unchanged. No provider implementation, provider selection, KDF/AEAD execution, key generation, storage, or production authorization is added; production selection remains `DisabledVaultCryptoProvider` only and `productionProviderSelectable=false`.
