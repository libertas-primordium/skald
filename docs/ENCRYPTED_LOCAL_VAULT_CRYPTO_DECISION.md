...
| Container format | Versioned vault container with plaintext unlock header and encrypted catalog/records; a still-disabled in-memory parser/writer now validates caller-supplied byte arrays and fixed non-secret fixtures, but no file-backed persistence path exists. |
