...
This is the last fast planning pass before slower test-only implementation work. It defines the narrow future plan only; current plan evidence does not implement a provider identity, does not authorize provider selection, and does not admit current implementation.
...
These artifacts are not present in this branch and do not authorize current implementation.
