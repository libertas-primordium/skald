...
Except for the separate commonTest-only inert marker, inert inventory, inert profile, inert validation report, inert reachability proof, inert capability matrix, and inert KAT fixture scope metadata, these artifacts remain future-only and do not authorize current implementation.
