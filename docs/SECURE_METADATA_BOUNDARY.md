...
The encrypted vault storage readiness decision is documented in [`ENCRYPTED_LOCAL_VAULT_STORAGE_READINESS_DECISION.md`](ENCRYPTED_LOCAL_VAULT_STORAGE_READINESS_DECISION.md), the container-format v1 decision is documented in [`ENCRYPTED_LOCAL_VAULT_CONTAINER_FORMAT_V1_DECISION.md`](ENCRYPTED_LOCAL_VAULT_CONTAINER_FORMAT_V1_DECISION.md), the storage path/session lifecycle decision is documented in [`ENCRYPTED_LOCAL_VAULT_STORAGE_PATH_SESSION_LIFECYCLE_DECISION.md`](ENCRYPTED_LOCAL_VAULT_STORAGE_PATH_SESSION_LIFECYCLE_DECISION.md), the migration/corruption policy decision is documented in [`ENCRYPTED_LOCAL_VAULT_MIGRATION_CORRUPTION_POLICY_DECISION.md`](ENCRYPTED_LOCAL_VAULT_MIGRATION_CORRUPTION_POLICY_DECISION.md), and the parser/writer admission gate is documented in [`ENCRYPTED_LOCAL_VAULT_PARSER_WRITER_ADMISSION_GATE.md`](ENCRYPTED_LOCAL_VAULT_PARSER_WRITER_ADMISSION_GATE.md). They admit only later separate design/implementation branches; secure metadata storage remains disabled, no secure metadata success path exists, no parser/writer implementation, serialization/parsing, vault bytes, migration execution, corruption detection/repair, backup creation, rollback, atomic replace, directory/file I/O, lock/unlock/session handling, KDF/AEAD/encryption/decryption, key/nonce generation, or Tink keyset creation/persistence is added, no production observation/address-index/UTXO/wallet-history persistence exists, and no production sync, provider selection, signing/broadcasting, UI, endpoint, or mainnet behavior is enabled.
...
A still-disabled runtime randomness authorization boundary now models future randomness operation kinds, purposes, source kinds, required gates, blockers, warnings, disabled capabilities, and redacted tokens for metadata encryption, metadata nonces, storage-index nonces, manifest nonces, backup/export material, migration/recovery planning, provider randomness, OS cryptographic randomness, Android entropy evidence, and Linux entropy evidence. Current authorization is blocked/fail-closed. The boundary accepts typed policy requests and typed evidence only. It does not accept random bytes, entropy bytes, salt bytes, nonce bytes, key bytes, seed bytes, passphrases, provider handles, provider instances, KDF input or output, AEAD keys or tags, ciphertext, plaintext, record bytes, manifest bytes, storage-index bytes, persisted container bytes, paths, Settings values, backend URLs, descriptors, credentials, wallet labels, UTXO labels, transaction notes, or wallet database bytes. It does not call `SecureRandom`, Kotlin Random, Java Random, `Math.random`, OS CSPRNG APIs, provider randomness APIs, or runtime randomness health checks. It does not generate entropy, salts, nonces, keys, random byte fixtures, deterministic vectors, provider operations, KATs, Argon2id/KDF/HKDF/HMAC, AEAD, header commitment logic, record crypto, key wrapping, provider clear/dispose, vault creation, vault unlock, vault persistence, provider selection, or mainnet. General-purpose PRNGs are forbidden for secrets, salts, nonces, and keys; future acceptable randomness must come from reviewed OS cryptographic randomness/CSPRNG or reviewed provider randomness. Android OS CSPRNG use remains future-reviewed only, Android hardware-backed key protection is separate from entropy quality, Linux entropy quality remains a required review gate, provider selection still selects only `DisabledVaultCryptoProvider`, and `productionProviderSelectable` remains false.
...
- runtime randomness authorization boundary evidence as runtime randomness authorization, OS CSPRNG readiness, provider randomness readiness, hardware-backed entropy readiness, Android randomness readiness, Linux randomness readiness, entropy generation readiness, salt generation readiness, nonce generation readiness, key-generation readiness, KDF salt readiness, AEAD nonce readiness, deterministic-vector production readiness, provider-operation approval, provider selectability, metadata encryption/decryption readiness, metadata persistence approval, or mainnet approval,
- creation authorization boundary evidence as vault creation authorization, creation attempt availability, initial passphrase acceptance, initial key material availability, salt/nonce/key/id generation readiness, KDF readiness, provider readiness, header/container/manifest/storage-index/record creation readiness, secure metadata creation readiness, wrapped-key storage readiness, storage namespace creation readiness, persistence commit approval, post-create unlock approval, active-session availability, decrypted-key availability, provider selectability, metadata persistence approval, or mainnet approval,
...
The still-disabled runtime randomness authorization boundary is covered only as typed policy evidence. It models future runtime randomness operation kinds, purposes, source kinds, required gates, blockers, warnings, disabled capabilities, and redacted tokens. It does not accept random, entropy, salt, nonce, key, seed, KDF, AEAD, ciphertext, plaintext, record, manifest, storage-index, persisted-container, provider, path, Settings, credential, wallet-label, UTXO-label, transaction-note, descriptor, or database values. It does not call `SecureRandom`, Kotlin Random, Java Random, `Math.random`, OS CSPRNG APIs, provider randomness APIs, or runtime randomness health checks, and it does not generate entropy, salts, nonces, keys, random fixtures, deterministic vectors, provider operations, KATs, Argon2id/KDF/HKDF/HMAC, AEAD, header commitment logic, record crypto, key wrapping, provider clear/dispose, provider selection, vault unlock, metadata persistence, or mainnet.

The still-disabled `SkaldVaultV1KdfCalibrationAuthorizationPolicy` boundary is covered only as typed policy evidence. It models future KDF operation kinds, purposes, parameter/evidence kinds, platform/device classes, required gates, blockers, warnings, disabled capabilities, and redacted tokens. It does not accept passphrases, salt bytes, KDF input/output, benchmark logs with host details, device identifiers, random bytes, entropy bytes, provider handles, keys, ciphertext, plaintext, records, manifests, storage indexes, persisted containers, paths, Settings values, credentials, descriptors, or database handles. It does not run Argon2id, run KDFs, run calibration, run benchmarks, inspect real host/device details, approve final KDF parameters, normalize or encode real passphrases, generate or consume salts, call randomness APIs, run provider operations, run provider KATs, derive vault keys, approve metadata key derivation, enable unlock, enable metadata persistence, make a provider selectable, or approve mainnet.

The still-disabled `SkaldVaultV1SecureStorageAuthorizationPolicy` boundary is covered only as typed policy evidence. It models future secure-storage operation kinds, secure value kinds, target kinds, required gates, blockers, warnings, disabled capabilities, and redacted tokens for sensitive metadata, manifest metadata, storage-index metadata, record descriptors, recovery metadata, backup/export material, session-adjacent state, app-controlled encrypted vault storage, Android optional wrapping, Linux optional wrapping, OS keyring targets, password-manager targets, Settings targets, plaintext targets, and test-only targets. Current secure-storage authorization is blocked/fail-closed because persistence readiness is blocked, secure secret storage and secure metadata storage are unavailable, provider operations are unauthorized, runtime randomness authorization is blocked, KDF calibration authorization is blocked, and no target is approved for production secret or metadata storage. It does not store or retrieve metadata, store or retrieve secrets, wrap or unwrap keys, export or import backup material, migrate or purge secure storage, use Android Keystore, Android Credential Manager, OS keyrings, password managers, SharedPreferences, Settings storage, files, databases, or encrypted local vault storage, run provider operations, run KDF/HKDF/HMAC/AEAD, call randomness APIs, enable unlock, enable metadata persistence, make a provider selectable, or approve mainnet.

The still-disabled `SkaldVaultV1UnlockAuthorizationPolicy` boundary is also typed policy evidence only. It models future unlock operation kinds, purposes, credential classes, required gates, blockers, warnings, disabled capabilities, and redacted tokens before secure metadata can ever be decrypted for an active vault session. Current unlock authorization is blocked/fail-closed because passphrase input is blocked, KDF calibration authorization is blocked, runtime randomness authorization is blocked, provider operations are unauthorized, secure-storage authorization is blocked, storage service operations are disabled, lock/session lifecycle is unavailable, persistence readiness is blocked, and provider selection still selects only `DisabledVaultCryptoProvider`.

The unlock authorization boundary does not accept passphrases, PINs, or biometrics; normalize or encode passphrases; run Argon2id/KDF/HKDF/HMAC/AEAD; generate or consume salts, nonces, or random bytes; call provider operations; read secure storage, metadata storage, or encrypted vault storage; retrieve or unwrap wrapped keys; decrypt metadata or records; create active sessions; hold decrypted key material; persist unlock state; add UI; enable vault creation, vault unlock, metadata persistence, production provider use, or mainnet. OS keyrings remain rejected as primary storage and for Skald-managed vault passphrase storage, password managers remain rejected for Skald-managed vault passphrase storage, Settings/preferences storage remains rejected for secrets and unlock state, provider selection still selects only `DisabledVaultCryptoProvider`, `productionProviderSelectable` remains false, and passphrase-first remains the default authority.

The still-disabled `SkaldVaultV1CreationAuthorizationPolicy` boundary is also typed policy evidence only. It models future creation operation kinds, purposes, initializer classes, required gates, blockers, warnings, disabled capabilities, and redacted tokens before secure metadata can ever be created for a new vault. Current creation authorization is blocked/fail-closed because passphrase input is blocked, KDF calibration authorization is blocked, runtime randomness authorization is blocked, provider operations are unauthorized, secure-storage authorization is blocked, storage service operations are disabled, storage safety is not runtime-approved, persistence readiness is blocked, unlock authorization is blocked, and provider selection still selects only `DisabledVaultCryptoProvider`.

The creation authorization boundary does not accept passphrases, PINs, or biometrics; normalize or encode passphrases; generate salts, nonces, keys, or ids; run Argon2id/KDF/HKDF/HMAC/AEAD; call provider operations; create headers, header commitments, containers, manifests, storage indexes, records, secure metadata, wrapped keys, storage namespaces, persistence commits, or active sessions; write secure storage, metadata storage, encrypted vault storage, Settings, files, or databases; add UI; enable vault creation, vault unlock, metadata persistence, production provider use, provider selectability, or mainnet. OS keyrings remain rejected as primary storage and for Skald-managed vault passphrase storage, password managers remain rejected for Skald-managed vault passphrase storage, Settings/preferences storage remains rejected for secrets, creation state, and unlock state, `productionProviderSelectable` remains false, and passphrase-first remains the default authority.

The still-disabled `SkaldVaultV1AuthorizationReadinessMatrixPolicy` boundary is typed policy evidence only. It consolidates the current blockers for secure metadata storage, secure secret storage, encrypted local vault storage, vault creation, vault unlock, active sessions, persistence, provider selection, provider operations, runtime randomness, KDF calibration, storage service operations, platform gates, wallet work, and mainnet.

The authorization/readiness matrix does not enable metadata persistence or any other capability. Every production/runtime capability remains blocked, warning-only evidence cannot authorize production use, user consent cannot override missing hard gates, test-only evidence cannot authorize production runtime, mainnet remains disabled, provider selection still selects only `DisabledVaultCryptoProvider`, and `productionProviderSelectable` remains false.
