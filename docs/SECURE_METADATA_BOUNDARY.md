...
The next focused pass should run the dependency spike for Argon2id/XChaCha20-Poly1305 support or refine vault readiness after review before any production sync or observation persistence is enabled. Do not add plaintext observation storage, production backend clients, address persistence, signing, broadcasting, public endpoints, or mainnet as part of that work.
