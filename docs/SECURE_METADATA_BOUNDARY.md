...
The encrypted vault storage readiness decision is documented in [`ENCRYPTED_LOCAL_VAULT_STORAGE_READINESS_DECISION.md`](ENCRYPTED_LOCAL_VAULT_STORAGE_READINESS_DECISION.md), and the container-format v1 decision is documented in [`ENCRYPTED_LOCAL_VAULT_CONTAINER_FORMAT_V1_DECISION.md`](ENCRYPTED_LOCAL_VAULT_CONTAINER_FORMAT_V1_DECISION.md). They admit only later separate design/implementation branches; secure metadata storage remains disabled, no secure metadata success path exists, no production observation/address-index/UTXO/wallet-history persistence exists, and no production sync, provider selection, signing/broadcasting, UI, endpoint, or mainnet behavior is enabled.
