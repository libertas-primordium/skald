...
The next focused pass should add Android runtime KAT validation for the Tink/Bouncy Castle dependency probe, evaluate IonSpin KMP libsodium packaging/KAT mapping in isolation, investigate a specific Lazysodium/JNA conflict strategy, or design a narrow disabled crypto-provider boundary before any production sync or observation persistence is enabled. Do not add plaintext observation storage, production backend clients, address persistence, signing, broadcasting, public endpoints, or mainnet as part of that work.
