...
`SkaldVaultV1CreationAuthorizationPolicy` now records the separate authorization gate that must be satisfied before any future Argon2id parameter set can be used for new vault creation. It models future creation operation kinds, purposes, initializer classes, required gates, blockers, warnings, disabled capabilities, and redacted policy tokens, but it authorizes no operation in this branch.
