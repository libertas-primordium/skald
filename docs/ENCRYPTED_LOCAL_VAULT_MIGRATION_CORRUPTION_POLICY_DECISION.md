...
The [`v1 canonical architecture decision`](ENCRYPTED_LOCAL_VAULT_V1_CANONICAL_ARCHITECTURE_DECISION.md) records the authoritative-manifest commit point, generation counter, previous-manifest commitment evidence, quarantine role, and the limitation that a fully consistent old snapshot cannot be detected without an external monotonic anchor. Prototype P0 bytes have no migration promise. No migration, repair, quarantine, rollback detection, atomic replace, or file I/O is implemented.
