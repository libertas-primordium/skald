...
- v1 production-provider acceptance contract with pinned Bouncy Castle Argon2id, Tink XChaCha20-Poly1305, OS SecureRandom, passphrase-first protection, optional Android wrapping, one pinned suite id, canonical header commitment prerequisites, `unicode-nfc-utf8-no-controls-no-whitespace-v1` passphrase encoding policy, Tink raw-key feasibility result `FEASIBLE_PUBLIC_RAW_KEY_API`, bounded Argon2id calibration gates, non-key-committing AEAD prerequisites, and disabled/non-selectable assessment results,
