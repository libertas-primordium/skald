...
- vault container, manifest, storage, atomicity/crash-recovery, secure-storage boundary, and anti-rollback-anchor contracts as model-only evidence that does not enable provider selectability or persistence,
...
- no provider-selectable implementation, vault container parser/reader/writer, manifest reader/writer, storage index reader/writer, filesystem/database/DataStore/SharedPreferences storage, secure-storage success path, or storage API added by the provider KAT/stale-record/container contracts.
