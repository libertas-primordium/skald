...
- vault container contract plus still-disabled in-memory parser/writer evidence, and manifest, storage, atomicity/crash-recovery, secure-storage boundary, and anti-rollback-anchor contracts as evidence that does not enable provider selectability or persistence,
- in-memory vault container fixture serialization/parsing with typed malformed-container failures and no file I/O, storage, KDF/HKDF/HMAC, or Tink AEAD execution,
