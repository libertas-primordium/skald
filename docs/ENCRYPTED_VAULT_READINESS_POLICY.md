...
These models are disabled and fail-closed. They encode the vault design and crypto decision record as typed policy state. The passphrase policy validator/NFC UTF-8 encoder, explicit-parameter Bouncy Castle Argon2id passphrase-to-root derivation, canonical header serializer, HKDF-SHA-256 expansion from caller-supplied root material, and HMAC-SHA-256 header commitment verification now exist as still-disabled building blocks, but the readiness policy still does not implement provider selectability, calibration, AEAD execution, vault creation, vault unlock, key generation, file writes, platform key stores, OS keyrings, secret persistence, sensitive metadata persistence, production sync, signing, broadcasting, Tor transport, or mainnet.
