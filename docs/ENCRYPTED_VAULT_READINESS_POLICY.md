...
- provider-level KAT contract categories, blockers, platform coverage, redaction requirements, and dependency-level KAT insufficiency,
...
The next focused pass should collect additional Android baseline Argon2id evidence or design a still-disabled executable provider test harness for the provider-level KAT contract, or evaluate an alternate KDF provider if the probe evidence is insufficient. IonSpin KMP libsodium packaging/KAT mapping and Lazysodium/JNA variant-resolution work remain separate replacement-stack probes if needed. Do not add production persistence, production sync, backend clients, signing, broadcasting, Tor transport, Nostr parsing, public endpoints, or mainnet as part of that work.
