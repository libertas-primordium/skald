...
The [`v1 canonical architecture decision`](ENCRYPTED_LOCAL_VAULT_V1_CANONICAL_ARCHITECTURE_DECISION.md) selects an encrypted authoritative manifest and sensitive-metadata policy but adds no persistence. Recovery/Privacy observation metadata, address-index state, UTXO state, wallet history, production sync, provider selection, signing/broadcasting, endpoints, and mainnet remain disabled and require separate implementation and release-review passes.
