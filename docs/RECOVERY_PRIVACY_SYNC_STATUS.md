...
Manual Android Argon2id calibration evidence capture is documented in [`ENCRYPTED_LOCAL_VAULT_ANDROID_CALIBRATION_CAPTURE.md`](ENCRYPTED_LOCAL_VAULT_ANDROID_CALIBRATION_CAPTURE.md). It can organize future device-class timing evidence, but it does not make recovery storage, privacy metadata persistence, provider selection, or production sync available.
