...
- The Tink raw-key feasibility probe is documented in [`ENCRYPTED_LOCAL_VAULT_TINK_RAW_KEY_FEASIBILITY_PROBE.md`](ENCRYPTED_LOCAL_VAULT_TINK_RAW_KEY_FEASIBILITY_PROBE.md) and reports `FEASIBLE_PUBLIC_RAW_KEY_API` for desktop/JVM test scope only. It does not approve production provider selection.
