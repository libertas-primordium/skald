...
`SkaldVaultV1ProviderKatExecutionIsolationPolicy` now exists as still-disabled model-only evidence above this harness. It proves the test-provider KAT harness concept, public non-wallet vector documentation, canonical header/HKDF/HMAC vector documentation, provider KAT contract evidence, provider selection, registry isolation, factory isolation, dispatch isolation, skeleton evidence, candidate packaging evidence, dependency/build evidence, promotion blockers, matrix evidence, creation authorization, unlock authorization, secure-storage authorization, and persistence readiness cannot become a non-disabled provider KAT executor or authorize production provider readiness. It does not implement a provider, add dependencies, instantiate provider code, add a provider factory, add a provider dispatcher, add a provider KAT executor, run provider operations, run KATs, run randomness checks, run KDF/HKDF/HMAC/AEAD, compute header commitments, encrypt/decrypt records, wrap/unwrap keys, enable vault creation/unlock/persistence, change provider selection, set `productionProviderSelectable=true`, or enable mainnet.
