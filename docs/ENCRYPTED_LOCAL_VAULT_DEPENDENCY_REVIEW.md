...
`VaultCryptoDependencyProbeCatalog` records the Tink plus Bouncy Castle split stack as `DependencyLicenseAndKeysetReviewCompleteCandidate` with `DisabledProviderBoundaryModeled`, `ProviderSelectionBoundaryModeled`, `ProviderKatContractModeled`, `Argon2idCandidateParameterPolicyModeled`, `AndroidArgon2idCalibrationEvidenceCaptureModeled`, `AndroidCompatibilityEntropyPolicyModeled`, and desktop plus Android test-scope Tink raw-key public API feasibility probe results.
...
- the desktop/JVM Tink raw-key feasibility probe records `FEASIBLE_PUBLIC_RAW_KEY_API` and the Android instrumented probe records `ANDROID_FEASIBLE_PUBLIC_RAW_KEY_API`: public Tink APIs can construct the pinned XChaCha20-Poly1305 primitive from caller-supplied fixed raw key bytes through a transient in-memory keyset handle on both targets, with no persisted Tink keyset, no random Tink-generated vault key, no key rotation, no multiple active keys, no internal APIs, and no reflection.
...
- Production key handling around the feasible raw-key API path. The current probes prove only that pinned Tink public APIs can construct the primitive from caller-supplied fixed raw key bytes in desktop/JVM and Android test scope; they do not approve production AEAD execution, key lifecycle, zeroization, or storage.
