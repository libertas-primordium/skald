...
The raw-key feasibility review now has a desktop/JVM test-scope outcome of `FEASIBLE_PUBLIC_RAW_KEY_API`. The probe uses fixed non-secret bytes and public Tink APIs to build a transient in-memory primitive from caller-supplied key bytes. It does not add production AEAD execution, does not persist a Tink keyset, does not generate a Tink vault key, does not use internal APIs, and does not remove the header-commitment requirement.
