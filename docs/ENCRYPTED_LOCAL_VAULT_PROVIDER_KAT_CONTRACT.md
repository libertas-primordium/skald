...
The full container/manifest/storage contract is now specified in [`ENCRYPTED_LOCAL_VAULT_CONTAINER_MANIFEST_STORAGE_CONTRACT.md`](ENCRYPTED_LOCAL_VAULT_CONTAINER_MANIFEST_STORAGE_CONTRACT.md). This branch does not implement a manifest reader, manifest writer, storage index, vault container parser, vault container writer, persistence path, conflict resolver, sync path, or anti-rollback anchor. Provider selectability remains blocked until manifest/storage stale-record policy is implemented and tested and crash/corruption/partial-write behavior is reviewed.
