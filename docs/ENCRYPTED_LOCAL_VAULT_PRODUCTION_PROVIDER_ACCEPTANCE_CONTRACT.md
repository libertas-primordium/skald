...
43. Safe path-construction contract is approved: future paths combine only a reviewed root and validated/encoded relative segments, reject absolute/dot/traversal/unsafe/secret-looking/user-label segments, prove containment under the reviewed root, and fail closed if containment cannot be proven.
43. Symlink/traversal contract is approved: future implementation avoids or rejects symlink traversal where possible, verifies resolved targets under the root where supported, fails closed on unknown symlink or containment state, and does not follow attacker-controlled links.
44. Storage permission/ownership contract is approved: future implementation prefers app-private OS isolation, rejects obviously unsafe permissions where detectable, documents Android and Linux expectations, and does not store vault data in world-readable or shared directories.
45. Durability capability contract is approved: future implementation documents and tests atomic replace, durable sync, parent-directory sync or equivalents, write ordering, unsupported-primitive fallback, and Android/desktop differences before persistence.
46. Durability fail-closed policy is approved: unknown, unsupported, unreviewed, insufficient, unsafe, or failed durability/root/permission/storage state blocks encrypted vault persistence unless a future explicitly reviewed equivalent safe strategy is approved.
47. Warning-only encrypted vault persistence is rejected: user consent cannot override required durability failure for v1 vault writes.
46. Runtime randomness uses OS SecureRandom with provider/algorithm evidence.
47. Unknown randomness/provider state blocks vault creation.
48. Forbidden random APIs remain guarded.
49. Secure secret storage is reviewed and approved.
50. Secure metadata storage is reviewed and approved.
51. Crash, corruption, and partial-write behavior are reviewed.
52. Redaction, logging, and crash-report leakage checks pass.
53. Android optional wrapping remains separate from entropy/randomness and passphrase recovery.
54. The still-disabled provider facade is approved as metadata/status only and remains non-selectable.
55. A production provider implementation exists behind Skald-owned interfaces.
56. Release readiness excludes debug and test-only providers from selection.
