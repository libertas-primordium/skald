...
10. raw-key handling approval through public supported Tink APIs or a separate human-approved alternative,
11. canonical header encoding integration that matches the non-secret test vectors,
12. canonical header, HKDF-SHA-256, and HMAC-SHA-256 provider-level tests matching the non-secret vectors,
13. HKDF-SHA-256 key expansion and HMAC-SHA-256 header commitment wired and tested through the still-disabled provider,
14. key-separation implementation with the selected key-expansion primitive,
15. strict AAD implementation and tamper/copy/replay tests,
16. provider-level deterministic vector KATs and randomized AEAD behavioral KATs on desktop and Android,
17. integrated verification-order KATs proving header commitment before record decrypt,
18. stale-record manifest/storage policy implementation and tests,
19. secure storage, secure metadata, and lock/session lifecycle approval,
20. vault container/storage review,
21. manifest/storage atomicity and crash-recovery review, including the still-disabled simulator evidence, separate platform durability review, and proof that unsupported/unknown/failed durability blocks persistence rather than warning-only continuation,
22. redaction and failure-mode tests,
23. migration/corruption tests,
24. explicit mainnet release-hardening if mainnet is requested.
