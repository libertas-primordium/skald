...
| Keyset or raw-key handling approved | Partially evidenced. The desktop/JVM test-scope result is `FEASIBLE_PUBLIC_RAW_KEY_API`, but no production provider implementation, Android raw-key probe, production provider KATs, header commitment, or storage review is approved. No keyset persistence is approved. |
