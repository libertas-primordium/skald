...
| Vault container/manifest/storage contract modeled | Evidence exists as contract ids: `skald-vault-v1-container-contract-v1`, `skald-vault-v1-manifest-contract-v1`, `skald-vault-v1-local-manifest-storage-policy-v1`, `skald-vault-v1-storage-namespace-path-policy-v1`, `skald-vault-v1-atomicity-crash-recovery-policy-v1`, `skald-vault-v1-secure-storage-boundary-policy-v1`, and `skald-vault-v1-anti-rollback-anchor-policy-v1`. The container parser/writer and namespace/path policy are implemented only for in-memory/caller-supplied values and fixed non-secret fixtures. This does not enable persistence or path construction. |
...
20. manifest/storage atomicity and crash-recovery review, including the still-disabled simulator evidence and separate platform durability review,
