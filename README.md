...
- The app-controlled encrypted local vault architecture and key-lifecycle plan is documented in [`docs/ENCRYPTED_LOCAL_VAULT_DESIGN.md`](docs/ENCRYPTED_LOCAL_VAULT_DESIGN.md). It is design-only. It does not add encryption code, crypto dependencies, production secret persistence, production metadata persistence, production sync, integrated Tor, external Tor connections, or mainnet. OS keyrings are not primary storage; they may later wrap vault keys only after explicit design review.
