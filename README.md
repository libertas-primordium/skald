...
- The app-controlled encrypted local vault architecture and key-lifecycle plan is documented in [`docs/ENCRYPTED_LOCAL_VAULT_DESIGN.md`](docs/ENCRYPTED_LOCAL_VAULT_DESIGN.md), with crypto/key-lifecycle decisions recorded in [`docs/ENCRYPTED_LOCAL_VAULT_CRYPTO_DECISION.md`](docs/ENCRYPTED_LOCAL_VAULT_CRYPTO_DECISION.md). Code-level readiness models are documented in [`docs/ENCRYPTED_VAULT_READINESS_POLICY.md`](docs/ENCRYPTED_VAULT_READINESS_POLICY.md). The focused dependency spike is documented in [`docs/ENCRYPTED_LOCAL_VAULT_DEPENDENCY_SPIKE.md`](docs/ENCRYPTED_LOCAL_VAULT_DEPENDENCY_SPIKE.md): pinned Tink and Bouncy Castle platform probes expose candidate XChaCha20-Poly1305 and Argon2id APIs, but they do not implement encryption, key derivation, vault containers, production secret persistence, production metadata persistence, production sync, integrated Tor, external Tor connections, or mainnet. OS keyrings are not primary storage; they may later wrap vault keys only after explicit design review.
...
- `docs/ENCRYPTED_VAULT_READINESS_POLICY.md` records the code-level encrypted vault readiness models. The models report the vault as not implemented, dependency selection unresolved beyond the packaging probe, KDF/AEAD parameters unverified, storage disabled, production persistence blocked, and mainnet disabled. They do not implement encryption, storage, unlock UI, or runtime cryptography.
