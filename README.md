...
Current status: initial Kotlin Multiplatform / Compose Multiplatform scaffold with Phase 1 descriptor-native on-chain domain modeling and non-secret Bitcoin backend profile settings started. This repository does not yet implement real wallet functionality. Do not use it with real funds.
...
- `settings` holds a small repository/codec for user-editable, persistent, non-secret Bitcoin backend profiles. Android uses SharedPreferences and desktop uses a local config file; both store only labels, backend type, development network, non-secret endpoint fields, and trust classification.
