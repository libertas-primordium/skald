...
- The app-controlled encrypted local vault architecture and key-lifecycle plan is documented in [`docs/ENCRYPTED_LOCAL_VAULT_DESIGN.md`](docs/ENCRYPTED_LOCAL_VAULT_DESIGN.md), with crypto/key-lifecycle decisions recorded in [`docs/ENCRYPTED_LOCAL_VAULT_CRYPTO_DECISION.md`](docs/ENCRYPTED_LOCAL_VAULT_CRYPTO_DECISION.md). Code-level readiness models are documented in [`docs/ENCRYPTED_VAULT_READINESS_POLICY.md`](docs/ENCRYPTED_VAULT_READINESS_POLICY.md). These docs and models are design/policy only. They do not add encryption code, crypto dependencies, production secret persistence, production metadata persistence, production sync, integrated Tor, external Tor connections, or mainnet. OS keyrings are not primary storage; they may later wrap vault keys only after explicit design review.
...
- `docs/ENCRYPTED_LOCAL_VAULT_CRYPTO_DECISION.md` records the vault crypto/key-lifecycle design decisions: Argon2id as the target KDF, XChaCha20-Poly1305 as the target record AEAD pending dependency review, per-record random nonce policy, key hierarchy, platform wrapping policy, container/versioning strategy, backup/export strategy, corruption/migration behavior, and readiness gates. It is a design record only; it does not add crypto dependencies or runtime cryptography.
- `docs/ENCRYPTED_VAULT_READINESS_POLICY.md` records the code-level encrypted vault readiness models. The models report the vault as not implemented, dependencies unresolved, KDF/AEAD parameters unverified, storage disabled, production persistence blocked, and mainnet disabled. They do not implement encryption, storage, unlock UI, or runtime cryptography.
