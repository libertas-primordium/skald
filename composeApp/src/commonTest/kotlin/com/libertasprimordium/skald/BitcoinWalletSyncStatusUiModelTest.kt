...
class BitcoinWalletSyncStatusUiModelTest {
    ...
    fun disabledSyncStatusModelProducesExpectedBlockerLabels() {
        ...
        assertContains(model.blockers, "Encrypted vault readiness is unavailable.")
        assertContains(model.blockers, "Secure metadata persistence is unavailable.")
        assertContains(model.blockers, "Observation persistence is unavailable.")
        assertContains(model.blockers, "Address index persistence is unavailable.")
        assertContains(model.warnings, "No network connection is attempted.")
        assertContains(model.warnings, "No sensitive wallet metadata is persisted.")
        assertContains(model.warnings, "Encrypted vault implementation is unavailable.")
        assertContains(model.warnings, "Encrypted vault readiness is policy-only.")
        assertContains(model.warnings, "Sensitive wallet metadata requires encrypted vault storage.")
